...
// ************* Extensions *************
// Declaramos una extensión de la clase Student
extension Student {
    
    // En una extension no podemos añadir stored properties
    // var age: Int
...
// Hacemos que la clase Teacher conforme el protocolo PersonName
// esto obliga a implementar la variable 'fullName' y la función 'isEqualTo()'
extension Teacher: PersonName {
    var fullName: String {
        get {
            return "\(name) \(lastname)"
        }
        set {
            // 'newValue' es el nuevo valor que se ha asignado a la variable
    ...
    func isEqualTo(other: String) -> Bool {
        return name.compare(other) == .orderedSame
    }
}

// Hacemos que la clase Student conforme el protocolo PersonName
// esto obliga a implementar la variable 'fullName' y la función 'isEqualTo()'
extension Student: PersonName {
    var fullName: String {
        get {
            return "\(name) \(lastname)"
        }
...
// 'student' y 'teacherDavid' son variable que ya hemos declarado
// Como las dos clases conforman el protocolo PersonName, las podemos
// añadir al mismo listado de datos
var persons: [PersonName] = [student, teacherDavid]
var person = persons.first

person?.isEqualTo(other: "")

student.fullName
student.fullName = "Natalia Soria"
student.fullName
student.name
student.lastname
...
class Coche: Vehiculo {
    ...
    // Sobreescribímos la funcionalidad de acelerar()
    override func acelerar() {
        velocidad += 30
    }
    
    override func frenar() {
        if(velocidad > 30) {
            // Llamamos a la función frenar() de la clase padre
            // de Vehículo
            super.frenar()
        }
    }
